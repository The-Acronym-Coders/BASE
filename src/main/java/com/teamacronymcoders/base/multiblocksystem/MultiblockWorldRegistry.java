...
final class MultiblockWorldRegistry {
	...
	public void processMultiblockChanges() {
		...
		// Merge pools - sets of adjacent machines which should be merged later on in
		// processing
		List<Set<MultiblockControllerBase>> mergePools = null;
		if (orphanedParts.size() > 0) {
			Set<IMultiblockPart> orphansToProcess = null;

			// Keep the synchronized block small. We can't iterate over orphanedParts
			// directly
			// because the client does not know which chunks are actually loaded, so
			// attachToNeighbors()
			...
			if (orphansToProcess != null && orphansToProcess.size() > 0) {
				...
				for (IMultiblockPart orphan : orphansToProcess) {
					...
					if (getMultiblockPartFromWorld(worldObj, coord) != orphan) {
					...
					if (compatibleControllers == null) {
						...
						for (Set<MultiblockControllerBase> candidatePool : mergePools) {
							if (!Collections.disjoint(candidatePool, compatibleControllers)) {
								// They share at least one element, so that means they will all touch after the
								// merge
		...
		if (mergePools != null && mergePools.size() > 0) {
			// Process merges - any machines that have been marked for merge should be
			// merged
			// into the "master" machine.
			// To do this, we combine lists of machines that are touching one another and
			// therefore
			// should voltron the fuck up.
			for (Set<MultiblockControllerBase> mergePool : mergePools) {
				// Search for the new master machine, which will take over all the blocks
		...
		// Any controllers which have had parts removed must be checked to see if some
		// parts are no longer
		...
		if (dirtyControllers.size() > 0) {
			...
			for (MultiblockControllerBase controller : dirtyControllers) {
				...
				if (newlyDetachedParts != null && newlyDetachedParts.size() > 0) {
					// Controller has shed some parts - add them to the detached list for delayed
					// processing
	...
	/**
	 * Called when a multiblock part is added to the world, either via chunk-load or
	 * user action. If its chunk is loaded, it will be processed during the next
	 * tick. If the chunk is not loaded, it will be added to a list of objects
	 * waiting for a chunkload.
	 *
	 * @param part
	...
	public void onPartAdded(final IMultiblockPart part) {
		...
		if (!worldObj.isBlockLoaded(worldLocation)) {
			...
			long chunkHash = Utils.getChunkXZHashFromBlock(worldLocation);
	...
	/**
	 * Called when a part is removed from the world, via user action or via chunk
	 * unloads. This part is removed from any lists in which it may be, and its
	 * machine is marked for recalculation.
	 *
	 * @param part
	...
	public void onPartRemovedFromWorld(final IMultiblockPart part) {
		...
		if (coord != null) {
			long hash = Utils.getChunkXZHashFromBlock(coord);
	...
	/**
	 * Called when the world which this World Registry represents is fully unloaded
	 * from the system. Does some housekeeping just to be nice.
	 */
	 ...
	 * awaiting load to the list of parts which are orphans and therefore will be
	 * added to machines after the next world tick.
	 *
	 * @param chunkX
	...
	/**
	 * Registers a controller as dead. It will be cleaned up at the end of the next
	 * world tick. Note that a controller must shed all of its blocks before being
	 * marked as dead, or the system will complain at you.
	 *
	 * @param deadController
	 *            The controller which is dead.
	 */
	public void addDeadController(MultiblockControllerBase deadController) {
		deadControllers.add(deadController);
	}

	/**
	 * Registers a controller as dirty - its list of attached blocks has changed,
	 * and it must be re-checked for assembly and, possibly, for orphans.
	 *
	 * @param dirtyController
	 *            The dirty controller.
	 */
	public void addDirtyController(MultiblockControllerBase dirtyController) {
		dirtyControllers.add(dirtyController);
	}
