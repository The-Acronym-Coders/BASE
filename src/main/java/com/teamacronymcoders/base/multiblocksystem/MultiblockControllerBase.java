...
public abstract class MultiblockControllerBase implements IMultiblockValidator {
    ...
    public final World controllerWorld;
    ...
    public void attachBlock(IMultiblockPart part) {
        ...
        if (!connectedParts.add(part))
            Base.instance.getLogger().warn(String.format(
                    ...
                    (controllerWorld.isRemote ? "CLIENT" : "SERVER"), hashCode(), part.hashCode(), coord));
        ...
        if (this.referenceCoord == null) {
            ...
            TileEntity te = this.controllerWorld.getTileEntity(referenceCoord);
        ...
        MultiblockRegistry.getInstance().addDirtyController(controllerWorld, this);
    }
    ...
    public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
        ...
        if (!connectedParts.remove(part)) {
            ...
            Base.instance.getLogger().warn(String.format(
                    ...
                    this.controllerWorld.isRemote ? "CLIENT" : "SERVER", part.hashCode(), position.getX(), position.getY(),
                    position.getZ()));
        }

        if (connectedParts.isEmpty()) {
            // Destroy/unregister
            MultiblockRegistry.getInstance().addDeadController(this.controllerWorld, this);
            return;
        }

        MultiblockRegistry.getInstance().addDirtyController(this.controllerWorld, this);
    ...
    private void _onAssimilated(MultiblockControllerBase otherController) {
        if (referenceCoord != null) {
            if (this.controllerWorld.isBlockLoaded(this.referenceCoord)) {
                TileEntity te = this.controllerWorld.getTileEntity(this.referenceCoord);
                if (te instanceof IMultiblockPart) {
                    ((IMultiblockPart) te).forfeitMultiblockSaveDelegate();
                }
            }
    ...
    public final void updateMultiblockEntity() {
        ...
        if (controllerWorld.isRemote) {
            ...
            if (minimumCoord != null && maximumCoord != null
                    && this.controllerWorld.isAreaLoaded(this.minimumCoord, this.maximumCoord)) {

                int minChunkX = this.getChunkXFromBlock(this.minimumCoord);
                int minChunkZ = this.getChunkZFromBlock(this.minimumCoord);
                int maxChunkX = this.getChunkXFromBlock(this.maximumCoord);
                int maxChunkZ = this.getChunkZFromBlock(this.maximumCoord);

                for (int x = minChunkX; x <= maxChunkX; x++) {
                    for (int z = minChunkZ; z <= maxChunkZ; z++) {
                        // Ensure that we save our data, even if the our save delegate is in has no TEs.
                        Chunk chunkToSave = this.controllerWorld.getChunk(x, z);
    ...
    public boolean shouldConsume(MultiblockControllerBase otherController) {
        ...
        if (res < 0) {
            ...
            Base.instance.getLogger().warn(String.format(
                    ...
                    this.controllerWorld.isRemote ? "CLIENT" : "SERVER"));
            auditParts();
            ...
            if (res < 0) {
                ...
                Base.instance.getLogger().fatal(String.format("My Controller (%d): size (%d), parts: %s", hashCode(),
                        this.connectedParts.size(), this.getPartsListString()));
                ...
                throw new IllegalArgumentException("[" + (this.controllerWorld.isRemote ? "CLIENT" : "SERVER")
    ...
    private void auditParts() {
        ...
        for (IMultiblockPart part : connectedParts) {
            if (part.isPartInvalid() || controllerWorld.getTileEntity(part.getWorldPosition()) != part) {
                onDetachBlock(part);
                deadParts.add(part);
            }
        }
        ...
        Base.instance.getLogger()
                .warn(String.format("[%s] Controller found %d dead parts during an audit, %d parts remain attached",
                        this.controllerWorld.isRemote ? "CLIENT" : "SERVER", deadParts.size(), this.connectedParts.size()));
    }
    ...
    public Set<IMultiblockPart> checkForDisconnections() {
        ...
        if (this.isEmpty()) {
            MultiblockRegistry.getInstance().addDeadController(controllerWorld, this);
            return null;
        }
        ...
        for (IMultiblockPart part : connectedParts) {
            ...
            if (!this.controllerWorld.isBlockLoaded(position) || part.isPartInvalid()) {
                deadParts.add(part);
                onDetachBlock(part);
                continue;
            }

            if (controllerWorld.getTileEntity(position) != part) {
                deadParts.add(part);
                onDetachBlock(part);
                continue;
            }
    ...
    public Set<IMultiblockPart> detachAllBlocks() {
        ...
        for (IMultiblockPart part : connectedParts) {
            if (this.controllerWorld.isBlockLoaded(part.getWorldPosition())) {
                onDetachBlock(part);
            }
        }
    ...
    private void selectNewReferenceCoord() {
        ...
        for (IMultiblockPart part : connectedParts) {
            position = part.getWorldPosition();
            if (part.isPartInvalid() || !this.controllerWorld.isBlockLoaded(position)) {
    ...
    protected void markReferenceCoordDirty() {
        if (controllerWorld == null || controllerWorld.isRemote) {
            return;
        }
        ...
        TileEntity saveTe = controllerWorld.getTileEntity(referenceCoord);
        controllerWorld.markChunkDirty(referenceCoord, saveTe);
    }
    ...
    protected void markMultiblockForRenderUpdate() {
        this.controllerWorld.markBlockRangeForRenderUpdate(this.getMinimumCoord(), this.getMaximumCoord());
    }
    ...
    private int getChunkXFromBlock(BlockPos position) {
        return position.getX() >> 4;
    }

    private int getChunkZFromBlock(BlockPos position) {
