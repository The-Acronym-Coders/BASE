 ...
 * associated group of TileEntities. Subordinate TileEntities implement the
 * IMultiblockPart class and, generally, should not have an update() loop.
 */
public abstract class MultiblockControllerBase implements IMultiblockValidator {
	...
	/**
	 * This is a deterministically-picked coordinate that identifies this multiblock
	 * uniquely in its dimension. Currently, this is the coord with the lowest X, Y
	 * and Z coordinates, in that order of evaluation. i.e. If something has a lower
	 * X but higher Y/Z coordinates, it will still be the reference. If something
	 * has the same X but a lower Y coordinate, it will be the reference. Etc.
	 */
	 ...
	 * The part will be notified that the data has been used after this call
	 * completes.
	 *
	 * @param part
	...
	public boolean hasBlock(BlockPos blockCoord) {
		if (WORLD.getTileEntity(blockCoord) instanceof IMultiblockPart) {
			return connectedParts.contains((IMultiblockPart)WORLD.getTileEntity(blockCoord));
		}
		return false;
	}
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if (!connectedParts.add(part)) {
			Base.instance.getLogger().warning(String.format(
	...
	/**
	 * Called when a new part is added to the machine. Good time to register things
	 * into lists.
	 *
	 * @param newPart
	...
	/**
	 * Called when a machine is paused from an assembled state This generally only
	 * happens due to chunk-loads and other "system" events.
	 */
	protected abstract void onMachinePaused();

	/**
	 * Called when a machine is disassembled from an assembled state. This happens
	 * due to user or in-game actions (e.g. explosions)
	 */
	protected abstract void onMachineDisassembled();

	/**
	 * Callback whenever a part is removed (or will very shortly be removed) from a
	 * controller. Do housekeeping/callbacks, also nulls min/max coords.
	 *
	 * @param part
	 *            The part being removed.
	 */
	 ...
	 *            Is this entity detaching due to the chunk unloading? If true, the
	 *            multiblock will be paused instead of broken.
	 */
	public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
		...
		if (!connectedParts.remove(part)) {
			...
			Base.instance.getLogger().warning(String.format(
					...
					WORLD.isRemote ? "CLIENT" : "SERVER", part.hashCode(), position.getX(), position.getY(),
					position.getZ()));
		}
	 ...
	 * blocks to actually assemble it. This isn't as simple as xmax*ymax*zmax for
	 * non-cubic machines or for machines with hollow/complex interiors.
	 *
	 * @return The minimum number of blocks connected to the machine for it to be
	 *         assembled.
	 */
	...
	/**
	 * @return the last validation error encountered when trying to assemble the
	...
	public void setLastError(String messageFormatStringResourceKey, Object... messageParameters) {
		lastValidationError = new ValidationError(messageFormatStringResourceKey, messageParameters);
	}

	/**
	 * Checks if a machine is whole. If not, set a validation error using
	 * IMultiblockValidator.
	 */
	protected abstract boolean isMachineWhole(IMultiblockValidator validatorCallback);

	/**
	 * Check if the machine is whole or not. If the machine was not whole, but now
	 * is, assemble the machine. If the machine was whole, but no longer is,
	...
	/**
	 * Called when the machine needs to be disassembled. It is not longer "whole"
	 * and should not be functional, usually as a result of a block being removed.
	...
	/**
	 * Assimilate another controller into this controller. Acquire all of the other
	 * controller's blocks and attach them to this one.
	 *
	 * @param other
	...
	public void assimilate(MultiblockControllerBase other) {
		...
		// releases all blocks and references gently so they can be incorporated into
		// another multiblock
	 ...
	 *            The controller consuming this controller.
	 */
	 ...
	 *
	 * @param assimilated
	 *            The controller whose uniqueness was added to our own.
	 */
	 ...
	 * controller. All blocks have been stripped out of this object and handed over
	 * to the other controller. This is intended primarily for cleanup.
	 *
	 * @param assimilator
	 *            The controller which has assimilated this controller.
	 */
	protected abstract void onAssimilated(MultiblockControllerBase assimilator);

	/**
	 * Driver for the update loop. If the machine is assembled, runs the game logic
	 * update method.
	 *
	 * @see MultiblockControllerBase#updateServer()
	 */
	public final void updateMultiblockEntity() {
		...
		if (WORLD.isRemote) {
			...
			if (minimumCoord != null && maximumCoord != null && WORLD.isAreaLoaded(minimumCoord, maximumCoord)) {
				...
				int minChunkX = Utils.getChunkXFromBlock(minimumCoord);
				int minChunkZ = Utils.getChunkZFromBlock(minimumCoord);
				int maxChunkX = Utils.getChunkXFromBlock(maximumCoord);
				int maxChunkZ = Utils.getChunkZFromBlock(maximumCoord);
	 ...
	 * loop. You do not need to call your superclass' update() if you're directly
	 * derived from MultiblockControllerBase. This is a callback. Note that this
	 * will only be called when the machine is assembled.
	 *
	 * @return True if the multiblock should save data, i.e. its internal game state
	 *         has changed. False otherwise.
	 */
	protected abstract boolean updateServer();

	/**
	 * Client-side update loop. Generally, this shouldn't do anything, but if you
	 * want to do some interpolation or something, do it here.
	 */
	 ...
	 *
	 * @param world
	 *            World object for the world in which this controller is located.
	 * @param x
	...
	/**
	 * Force this multiblock to recalculate its minimum and maximum coordinates from
	 * the list of connected parts.
	 */
	...
	/**
	 * Tests whether this multiblock should consume the other multiblock and become
	 * the new multiblock master when the two multiblocks are adjacent. Assumes both
	 * multiblocks are the same type.
	 *
	 * @param otherController
	 *            The other multiblock controller.
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if (res < 0) {
			...
			if (res < 0) {
				...
				Base.instance.getLogger().fatal(String.format("My Controller (%d): size (%d), parts: %s", hashCode(),
						connectedParts.size(), getPartsListString()));
				Base.instance.getLogger()
						.fatal(String.format("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(),
	...
	private int _shouldConsume(MultiblockControllerBase otherController) {
		...
		// Always consume other controllers if their reference coordinate is null - this
		// means they're empty and can be
	...
	/**
	 * Checks all of the parts in the controller. If any are dead or do not exist in
	 * the world, they are removed.
	 */
	private void auditParts() {
		...
		Base.instance.getLogger()
				.warning(String.format("[%s] Controller found %d dead parts during an audit, %d parts remain attached",
						WORLD.isRemote ? "CLIENT" : "SERVER", deadParts.size(), connectedParts.size()));
	}
	...
	public Set<IMultiblockPart> checkForDisconnections() {
		...
		if (referencePart == null || isEmpty()) {
			// There are no valid parts remaining. The entire multiblock was unloaded during
			// a chunk unload. Halt.
		...
		// Now visit all connected parts, breadth-first, starting from reference coord's
		// part
	...
	public Set<IMultiblockPart> detachAllBlocks() {
		...
		for (IMultiblockPart part : connectedParts) {
			if (WORLD.isBlockLoaded(part.getWorldPosition())) {
				onDetachBlock(part);
			}
		}
	...
	/**
	 * @return True if this multiblock machine is considered assembled and ready to
	 *         go.
	 */
	...
	/**
	 * Marks the reference coord dirty & updateable. On the server, this will mark
	 * the for a data-update, so that nearby clients will receive an updated
	 * description packet from the server after a short time. The block's chunk will
	 * also be marked dirty and the block's chunk will be saved to disk the next
	 * time chunks are saved. On the client, this will mark the block for a
	 * rendering update.
	 */
	protected void markReferenceCoordForUpdate() {
		...
		if ((WORLD != null) && (rc != null)) {
			Utils.notifyBlockUpdate(WORLD, rc, null, null);
		}
	}

	/**
	 * Marks the reference coord dirty. On the server, this marks the reference
	 * coord's chunk as dirty; the block (and chunk) will be saved to disk the next
	 * time chunks are saved. This does NOT mark it dirty for a description-packet
	 * update. On the client, does nothing.
	 *
	...
	/**
	 * Marks the whole multiblock for a render update on the client. On the server,
	 * this does nothing
	 */
