...
/**
 * Basic interface for a multiblock machine part. Preferably, you should derive
 * from MultiblockTileEntityBase, which does all the hard work for you.
...
public interface IMultiblockPart {
	...
	/**
	 * Called when this block is being orphaned. Use this to copy game-data values
	 * that should persist despite a machine being broken. This should NOT mark the
	 * part as disconnected. onDetached will be called immediately afterwards.
	 *
	 * @param oldController
	 *            The controller which is orphaning this block.
	 * @param oldControllerSize
	 *            The number of connected blocks in the controller prior to shedding
	 *            orphans.
	 * @param newControllerSize
	 *            The number of connected blocks in the controller after shedding
	 *            orphans.
	...
	/**
	 * Factory method. Creates a new multiblock controller and returns it. Does not
	 * attach this tile entity to it. Override this in your game code!
	 *
	...
	/**
	 * Retrieve the type of multiblock controller which governs this part. Used to
	 * ensure that incompatible multiblocks are not merged.
	 *
	 * @return The class/type of the multiblock controller which governs this type
	 *         of part.
	 */
	 ...
	 *
	 * @param newController
	 *            The new controller into which this tile entity is being merged.
	 */
	...
	/**
	 * Called when this block is no longer the designated block for saving data and
	 * transmitting data across the wire.
	 */
	...
	/**
	 * Returns an array containing references to neighboring IMultiblockPart tile
	 * entities. Primarily a utility method. Only works after tileentity
	 * construction, so it cannot be used in MultiblockControllerBase::attachBlock.
	 * This method is chunk-safe on the server; it will not query for parts in
	 * chunks that are unloaded. Note that no method is chunk-safe on the client,
	 * because ChunkProviderClient is stupid.
	 *
	 ...
	 * it was broken by a player/entity action, not by chunk unloads. Note that, for
	 * non-square machines, the min/max coordinates may not actually be part of the
	 * machine! They form an outer bounding box for the whole machine itself.
	 *
	...
	/**
	 * Called when the machine is broken for game reasons, e.g. a player removed a
	 * block or an explosion occurred.
	 */
	void onMachineBroken();

	/**
	 * Called when the user activates the machine. This is not called by default,
	 * but is included as most machines have this game-logical concept.
	 */
	void onMachineActivated();

	/**
	 * Called when the user deactivates the machine. This is not called by default,
	 * but is included as most machines have this game-logical concept.
	 */
	...
	/**
	 * Called when this part should check its neighbors. This method MUST NOT cause
	 * additional chunks to load. ALWAYS check to see if a chunk is loaded before
	 * querying for its tile entity This part should inform the controller that it
	 * is attaching at this time.
	 *
	 * @return A Set of multiblock controllers to which this object would like to
	 *         attach. It should have attached to one of the controllers in this
	 *         list. Return null if there are no compatible controllers nearby.
	 */
	Set<MultiblockControllerBase> attachToNeighbors();

	/**
	 * Assert that this part is detached. If not, log a warning and set the part's
	 * controller to null. Do NOT fire the full disconnection logic.
	 */
	void assertDetached();
	...
	/**
	 * @return The part's saved multiblock game-data in NBT format, or null if there
	 *         isn't any.
	 */
	NBTTagCompound getMultiblockSaveData();

	/**
	 * Called after a block is added and the controller has incorporated the part's
	 * saved multiblock game-data into itself. Generally, you should clear the saved
	 * data here.
	 */
